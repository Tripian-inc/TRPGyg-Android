...
fun String.getRequestStartDate(): String {
    return "${this}T00:00:00"
}
...
fun String.getRequestEndDate(): String {
    return "${this}T23:59:00"
}
...
fun getOfferDates(): OfferDateModel {
    val format: DateFormat = SimpleDateFormat("yyyy-MM-dd", Locale.ENGLISH)

    val today = Calendar.getInstance()
    today.time = Date()
    //today.add(Calendar.DATE, -1) //max 31 gün kuralına takılıyordu. Bu yüzden 1 gün çıkarıldı.

    val nextWeek = Calendar.getInstance()
    nextWeek.time = Date()
    nextWeek.add(Calendar.MONTH, 1)

    return OfferDateModel(format.format(today.time), format.format(nextWeek.time))
}


data class OfferDateModel constructor(
    val from: String,
    val to: String
) : BaseModel()

fun offerDate(dateText: String?): String {
    var sdf: SimpleDateFormat?
    val date: Date?
    ...
    if (dateText != null && dateText.isNotEmpty()) {
        try {
            ...
            sdf = SimpleDateFormat("dd MMM hh:mm aa", Locale.ENGLISH)
...
fun optInDate(dateText: String?): String {
    var sdf: SimpleDateFormat?
    val date: Date?
