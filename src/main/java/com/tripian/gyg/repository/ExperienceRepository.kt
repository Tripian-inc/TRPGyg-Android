...
object ExperienceRepository {
    ...
    fun getTours(language: String, currency: String, q: String?, categories: String?, limit: Int, dateFrom: String, dateTo: String): Flow<ExperiencesResponse> = sendRequest {
        service.getTours(language, currency, q, limit, categories = categories, dateFrom = dateFrom, dateTo = dateTo)
    }
    ...
    fun deleteBooking(shoppingCartHash: String, lang: String): Flow<BaseModel> = sendRequest {
        service.deleteBooking(shoppingCartHash, lang, "usd")
    }
    ...
    fun pay(request: PaymentRequest): Flow<PayResponse> = sendRequest {
        service.pay(request)
    }
