...
class FRTourDateVM : BaseViewModel() {
    ...
    fun init() {
        if (!::options.isInitialized) {
            if (!BookingPageData.data.date.isNullOrEmpty()) {
                calendar.timeInMillis = formatDate2Timestamp(BookingPageData.data.date, inputFormat = "yyyy-MM-dd")
            }
    ...
    fun onSelectedOption(option: BookOption) = launch {
        ...
        BookingPageData.data.pickUp = option.pickUp
        BookingPageData.data.language = option.language
        BookingPageData.data.parameters = option.parameters

        if (isOpenActivityDetails()) {
            navigateToFragment(FRActivityDetails.newInstance(), animation = AnimationType.ENTER_FROM_RIGHT)
        } else {
    ...
    private fun isOpenActivityDetails(): Boolean {
        return !BookingPageData.data.language?.audio.isNullOrEmpty() || !BookingPageData.data.language?.booklet.isNullOrEmpty() || !BookingPageData.data.language?.live.isNullOrEmpty() || !BookingPageData.data.parameters.isNullOrEmpty()
    }
}
